...
//     const storeValue = await getStorePlus(storeKey)
//     return async function () {
//         await setStorePlus(storeKey, storeValue)
//     }
// }
// setStorePlus辅助-创建还原点
function createRestorePoint(key,value) {
...
// setStorePlus辅助-恢复到最近的还原点
function tryRestoreToRestorePoint(key) {
    return new Promise((resolve, reject) => {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        chrome.storage.sync.get(keys, (valuesObj) => {
            ...
            try {
                ...
                // 返回的data是存储本地那份好的
                tryRestoreToRestorePoint(key).then(data=>resolve(data)).catch(()=>{
                    // 如果恢复失败，那就清理掉
                    removeStorePlus(key)
                    resolve(undefined);
                })
